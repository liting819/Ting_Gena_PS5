...
# Load the U.S. Attorney District shapefile (with all components present)
shapefile_path = "C:/Users/madig/Documents/Github/Year 2024-2025/Ting_Gena_PS5/shapefile/geo_export_8ce2e721-0b80-43a4-a309-9fb04d333f17.shp"
districts_shapefile = gpd.read_file(shapefile_path)

# Load your enforcement actions data from the Excel file
df = pd.read_excel("C:/Users/madig/Documents/Github/Year 2024-2025/Ting_Gena_PS5/df_2021.xlsx")

# Step 1: Extract and clean district names from the "Agency" column
df['District'] = df['Agency'].apply(lambda x: re.search(r'District of [A-Za-z ]+', x).group(0) if 'District' in x else None)
df_districts = df.dropna(subset=['District'])

# Step 2: Aggregate the number of enforcement actions by district
district_counts = df_districts['District'].value_counts().reset_index()
district_counts.columns = ['District', 'Enforcement_Count']

# Step 3: Clean the 'District' names in both datasets to ensure they match for merging
districts_shapefile['District'] = districts_shapefile['NAME'].str.replace(r'\s+District\s+of\s+', ' of ', regex=True)
district_counts['District'] = district_counts['District'].str.replace(r'\s+District\s+of\s+', ' of ', regex=True)

# Step 4: Merge the district counts with the shapefile based on district names
merged = districts_shapefile.merge(district_counts, on='District', how='left')
